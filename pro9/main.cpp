...
int main()
{
    Mat image = imread("/Users/Han/programming/opencv/pro9/color_space.jpg", 1);
    CV_Assert(!image.empty());

    // imshow("연습문제 17", image);

    Rect2d roi_img = selectROI(image); // Select ROI
    Mat imCrop = image(roi_img);       // Crop image
    imshow("선택영역", imCrop);

    Mat HSV_img, HSV_arr[3];
    cvtColor(imCrop, HSV_img, COLOR_BGR2HSV);
